...
export default function Dashboard() {
  ...
  const nextCap = mmr < 300 ? 300 : mmr < 700 ? 700 : mmr < 1200 ? 1200 : mmr < 1800 ? 1800 : mmr < 2500 ? 2500 : 3000;
  const prevCap = mmr < 300 ? 0 : mmr < 700 ? 300 : mmr < 1200 ? 700 : mmr < 1800 ? 1200 : mmr < 2500 ? 1800 : 2500;
  const progress = Math.min(100, Math.round(((mmr - prevCap) / (nextCap - prevCap)) * 100));
    ...
    <section className="container py-12 md:py-16">
      ...
      <div className="mt-8 grid md:grid-cols-3 gap-6">
        <div className="rounded-2xl border border-border/60 bg-secondary/40 backdrop-blur-md p-6">
          ...
          <div className="mt-2 text-sm text-foreground/70">{progress}% to next tier</div>
        </div>
        <div className="rounded-2xl border border-border/60 bg-secondary/40 backdrop-blur-md p-6">
          <h3 className="font-semibold mb-3">Rank</h3>
          <div className="text-3xl font-bold text-accent">{profile?.rankTier ?? "Rookie"}</div>
